...
export default async function DashboardPage() {
  ...
  const restyleList = (restyles ?? []) as Pick<Restyle, "id" | "title" | "current_url" | "original_url" | "updated_at" | "created_at">[];
  ...
  const projects: DashboardProject[] = restyleList
    .map((r): DashboardProject => ({
      ...
      thumb: r.current_url ?? r.original_url, date: r.updated_at,
      // "Reimagined" once the room has actually been generated (current diverges from the
      // canonicalized original); otherwise it's still a "Draft".
      reimagined: !!(r.current_url && r.original_url && r.current_url !== r.original_url),
    }))
