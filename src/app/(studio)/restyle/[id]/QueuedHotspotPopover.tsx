...
import { X, Replace, Plus, Trash2, Eraser } from "lucide-react";
...
export default function QueuedHotspotPopover({
  ...
  const isRemove = edit.kind === "remove";
  const below = cy <= 50;
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 p-3 pb-2">
        {!isRemove && edit.reference_url ? (
          ...
          <span className="h-14 w-14 rounded-xl bg-[var(--muted)] border border-[var(--border)] shrink-0 flex items-center justify-center text-[var(--muted-foreground)]">
            {isRemove ? <Eraser className="h-4 w-4" /> : isAdd ? <Plus className="h-4 w-4" /> : <Replace className="h-4 w-4" />}
          </span>
        )}
        <div className="min-w-0 flex-1 space-y-1">
          <span className="inline-block rounded-full bg-[var(--accent-soft)] text-[var(--accent-soft-foreground)] text-[10px] font-medium uppercase tracking-wide px-2 py-0.5">
            {isRemove ? "Will be removed" : "Queued"}
          </span>
          <p className="text-sm font-semibold capitalize leading-snug">{isRemove ? label : edit.product_title ?? label}</p>
          <p className="text-[11px] text-[var(--muted-foreground)]">
            {isRemove ? "Removed" : isAdd ? "Adding" : "Swapping"} · not in the photo yet
          </p>
        </div>
      ...
      <div className="flex gap-2 p-3 pt-0">
        {isRemove ? (
          onRemove && (
            <Button size="sm" variant="subtle" className="flex-1" onClick={onRemove}>
